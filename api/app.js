...
function cleanProduct(body,old={}){
  ...
  const product={id:String(body.id??old.id??'').trim()||`product-${Date.now()}`,name,price,description:String(body.description??old.description??'').trim(),image:images[0],images,status:['sold','available'].includes(String(body.status??old.status))?String(body.status??old.status):'available'};
